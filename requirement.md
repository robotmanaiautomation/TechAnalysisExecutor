...
### 차트 시각화
- **plotly**: 인터랙티브 차트 생성 (캔들스틱, 선 차트, 지표 오버레이)
  - API 응답: JSON (Plotly figure 객체) 또는 HTML (스탠드얼론 렌더링)
...
- `POST /api/bar/bulk-fetch` — **다중 티커 BAR 데이터 일괄 조회** (요청 body: 티커 목록, 응답: 티커별 BAR 데이터 전체)
  - PortfolioManager 등에서 보유 티커들의 BAR 데이터를 한 번에 취득 가능

### 기술적 분석
- `GET /api/analysis/{ticker}` — 기술적 지표 계산 결과 (JSON)
  - 지표 목록 파라미터: `?indicators=rsi,sma,macd`
  - 기간 파라미터: `?days=30` (기본값 30)
- `POST /api/analysis/bulk` — **다중 티커 기술적 분석 일괄 수행** (요청 body: 티커 목록 + 지표/기간, 응답: 티커별 분석 결과)

### 차트 시각화
- `GET /api/chart/{ticker}` — 차트 (JSON 또는 HTML)
  - 응답 형식: `?format=json` (Plotly figure 객체) / `?format=html` (스탠드얼론 HTML)
